    ...
    <div class="wrap">
      ...
      <section id="monitorPanel" class="monitor-panel" aria-label="Monitor game hari ini">
        <label>Master password
          <input id="viewerKey" type="password" placeholder="Masukkan master password" autocomplete="current-password">
        </label>
        <button id="loadGames" type="button">Lihat Game Hari Ini</button>
        <p class="fine">Akses khusus master admin. Link yang muncul mengikuti level password yang dimasukkan.</p>
...
document.getElementById("loadGames").addEventListener("click", async () => {
  ...
  const isAdminMode = data.mode === "admin";
  const modeCard = isAdminMode ? `
    <div class="link-card primary-link">
      <b>Mode super admin aktif.</b>
      <span>Link recovery admin di bawah bisa edit score.</span>
    </div>
  ` : "";
  box.innerHTML = modeCard + data.games.map(game => {
    const viewUrl = basePath() + game.view_url;
    const editUrl = game.edit_url ? basePath() + game.edit_url : "";
    return `
    <div class="link-card">
      <b>${game.title} - ${game.id}</b>
      <span>Update terakhir: ${game.updated_at}${game.expired ? " | Expired" : ""}${game.locked ? " | Locked" : ""}</span>
      ${isAdminMode && game.edit_url ? `
        <span>Recovery admin/edit score:</span>
        <a class="action-link" href="${game.edit_url}">Buka Admin / Edit Score</a>
        <a href="${game.edit_url}">${editUrl}</a>
      ` : ""}
      <span>View-only:</span>
      <a href="${game.view_url}">${viewUrl}</a>
    </div>
  `;
  }).join("");
});
