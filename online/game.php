...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .defaults { display: grid; gap: 7px; padding: 10px; border: 1px solid var(--line); border-radius: 8px; background: #fff; }
    ...
    .rule-row .amount { display: inline-grid; grid-template-columns: auto minmax(58px, auto); gap: 4px; align-items: center; justify-content: end; font-weight: 900; font-variant-numeric: tabular-nums; text-align: right; }
    ...
    .card { border: 1px solid var(--line); border-radius: 8px; padding: 11px; background: #fff; min-height: 90px; }
...
<body>
  <header>
    <div class="top">
      ...
      <div class="row">
        <a id="shareWhatsapp" class="share-wa hide" target="_blank" rel="noopener">Share to WhatsApp</a>
        <button id="openAcakHole" class="soft">Acak Hole</button>
        <button id="editSetup" class="soft hide">Edit Setup</button>
  ...
  <script>
    ...
    function uiDialog({
      ...
      return new Promise(resolve => { uiDialogResolver = resolve; });
    }
    ...
    function normalizeCoursePair(changedId) {
      ...
      const choices = ["valley", "lake", "hill"];
      const replacement = choices.find(course => course !== state[changedId]);
      if (changedId === "front") state.back = replacement;
      if (changedId === "back") state.front = replacement;
    }
    ...
    function recommendVoor(a, b, diff, holeIdx = lastPlayableHole()) {
      ...
      if (currentLoserGives) {
        ...
        return excess ? `${names()[winner]} ke ${names()[loser]}: NAIK ${excess}` : `${names()[loser]} ke ${names()[winner]}: TURUN Skret`;
      }
      const nextCount = currentWinnerGives + steps;
      return `${names()[winner]} ke ${names()[loser]}: NAIK ${nextCount}`;
    }
    function matchplayHtml(filterPlayer = "all") {
      ...
      for (let a = 0; a < state.players.length; a++) {
        for (let b = a + 1; b < state.players.length; b++) {
          ...
          const voorNow = winnerIdx === null ? "Skret" : actualVoorText(winnerIdx, loserIdx);
          rows.push(`<tr>
            ...
            <td>${esc(voorNow)}</td>
            <td>${m.diff === 0 ? "TETAP" : esc(recommendVoor(a, b, m.diff))}</td>
          </tr>`);
        }
      }
    ...
    async function pushLiveState(useKeepalive = false) {
      ...
      try {
        ...
        if (data.ok) {
          ...
          liveSaveStatus = `Live saved${Number.isFinite(data.voor_count) ? ` (Voor ${data.voor_count})` : ""}`;
          return true;
        }
    ...
    async function pushLiveSetup() {
      ...
      try {
        ...
        if (data.ok) {
          liveLastUpdated = data.updated_at || liveLastUpdated;
          liveSaveStatus = `Setup saved${Number.isFinite(data.voor_count) ? ` (Voor ${data.voor_count})` : ""}`;
          return true;
        }
    ...
    function updateShareWhatsapp() {
      ...
      const viewUrl = `${location.origin}/online/game.php?id=${encodeURIComponent(LIVE_GAME_ID)}`;
    ...
    function renderSetup() {
      ...
      document.getElementById("rateSummary").innerHTML = `
        ...
        <div class="rule-row"><span>1. Face to Face</span><span class="amount"><span>Rp</span><span>${fmt.format(state.rates.f2f)}</span></span></div>
        <div class="rule-row"><span>2. Single Winner</span><span class="amount"><span>Rp</span><span>${fmt.format(state.rates.winner)}</span></span></div>
        <div class="rule-row"><span>3. Winner (On BERDIE)</span><span class="amount"><span>Rp</span><span>${fmt.format(state.rates.par3Bonus)}</span></span></div>
        ${state.useBacarat ? `<div class="rule-row"><span>4. Bacarat Par 3</span><span class="amount"><span>Rp</span><span>${fmt.format(state.rates.bacaratPar3)}</span></span></div>
        <div class="rule-row"><span>Bacarat Non Par 3</span><span class="amount"><span>Rp</span><span>${fmt.format(state.rates.bacarat)}</span></span></div>
    ...
    function renderNineSummary() {
      ...
      document.getElementById("nineGrossCard").innerHTML = grossScorecardHtml(startIdx, endIdx);
    ...
    function grossScorecardHtml(startIdx, endIdx, showSplitTotals = false) {
      const holes = roundHoles().slice(startIdx, endIdx + 1);
      const ns = names();
      const courseNames = [...new Set(holes.map(h => h.course))].join(" - ");
      ...
      const totals = state.players.map((_, player) => holes.reduce((sum, hole, offset) => {
        const idx = startIdx + offset;
        if (!state.holes[idx]?.completed || !isActive(player, idx)) return sum;
        const rel = Number(state.holes[idx].scores[player]);
        return Number.isFinite(rel) ? sum + hole.par + rel : sum;
      }, 0));
      const splitHead = showSplitTotals ? `<th class="money score-total-col">Front 9</th><th class="money score-total-col">Back 9</th><th class="money score-grand-col">Total 18</th>` : "";
      const splitPar = showSplitTotals ? `<th class="money score-total-col">${roundHoles().slice(0, 9).reduce((sum, h) => sum + h.par, 0)}</th><th class="money score-total-col">${roundHoles().slice(9, 18).reduce((sum, h) => sum + h.par, 0)}</th><th class="money score-grand-col">${roundHoles().slice(0, 18).reduce((sum, h) => sum + h.par, 0)}</th>` : "";
      return `<table>
        <thead>
          <tr><th>Course</th><th>Player</th>${holes.map(h => `<th class="money">${h.global}</th>`).join("")}${splitHead}<th class="money score-grand-col">Gross Total</th></tr>
          <tr><th>${esc(courseNames)}</th><th>Par</th>${holes.map(h => `<th class="money">${h.par}</th>`).join("")}${splitPar}<th class="money score-grand-col">${holes.reduce((sum, h) => sum + h.par, 0)}</th></tr>
        </thead>
        <tbody>${ns.map((name, player) => `<tr><td>${esc(courseNames)}</td><td><b>${esc(name)}</b></td>${holes.map((hole, offset) => {
          const idx = startIdx + offset;
          const rel = Number(state.holes[idx]?.scores[player]);
          const gross = state.holes[idx]?.completed && isActive(player, idx) && Number.isFinite(rel) ? hole.par + rel : "";
          return `<td class="money">${gross}</td>`;
        }).join("")}${showSplitTotals ? `<td class="money score-total-col">${segmentTotal(player, 0, 8)}</td><td class="money score-total-col">${segmentTotal(player, 9, 17)}</td><td class="money score-grand-col">${segmentTotal(player, 0, 17)}</td>` : ""}<td class="money score-grand-col"><b>${totals[player] || ""}</b></td></tr>`).join("")}</tbody>
      </table>`;
    }
    function currentScorecardHtml() {
      ...
      return `<div class="hint"><b>${title}</b><br>${displayDate()} | Gross stroke only, tanpa detail betting.</div>
    ...
    function progressHtml(mode, options = {}) {
      ...
      return `${!options.hideCoffee && coffeeMessage() ? `<div class="hint"><b>${esc(coffeeMessage())}</b></div>` : ""}
        <div class="cards">${ns.map((n, i) => `<div class="card"><strong>${esc(n)}</strong><div class="total">${signed(c.totals[i].total)}</div><p>F2F ${fmt.format(c.totals[i].f2f)} | Single Winner ${fmt.format(c.totals[i].winner)}${state.useBacarat ? ` | Bacarat ${fmt.format(c.totals[i].bacarat)}` : ""} | Kring ${fmt.format(c.totals[i].kring)}</p></div>`).join("")}</div>
        <div class="table"><table><thead><tr><th>Player</th><th class="money">Face to face</th><th class="money">Single Winner</th><th class="money">On Berdie</th>${state.useBacarat ? `<th class="money">Bacarat</th>` : ""}<th class="money">Kring</th><th class="money">Total</th></tr></thead><tbody>${ns.map((n, i) => `<tr><td><b>${esc(n)}</b></td><td class="money">${signed(c.totals[i].f2f)}</td><td class="money">${signed(c.totals[i].winner)}</td><td class="money">${signed(c.totals[i].birdie)}</td>${state.useBacarat ? `<td class="money">${signed(c.totals[i].bacarat)}</td>` : ""}<td class="money">${signed(c.totals[i].kring)}</td><td class="money"><b>${signed(c.totals[i].total)}</b></td></tr>`).join("")}</tbody></table></div>
        ${achievementsHtml()}
        ${f2fDetail}`;
    }
    function ledgerHtml() {
    ...
    function blockViewerEdit(e) {
      ...
      if (button || target.matches?.("input, select, textarea")) {
        e.preventDefault();
        e.stopPropagation();
        uiAlert("Mode view-only. Untuk edit score gunakan link admin.", { title: "View only", kicker: "Akses terbatas" });
      }
    }
    ...
    document.addEventListener("focusout", async e => {
      ...
      if (t?.dataset?.voor) {
        ...
        if (normalized === null) {
          await uiAlert("Input voor tidak valid.\nGunakan angka seperti 1 atau 2, +1 atau +2, 0/kosong untuk Skret, atau x untuk No-Game.", { title: "Voor tidak valid" });
    ...
    document.addEventListener("change", async e => {
      ...
      if (t.id === "front" || t.id === "back") {
        if (state.started) {
          await uiAlert("Course tidak bisa diganti setelah game berjalan agar score yang sudah tersimpan tidak kacau.", { title: "Course terkunci", tone: "warn" });
          syncInputs();
          return;
        }
        state[t.id] = t.value;
    ...
    document.addEventListener("click", async e => {
      ...
      if (b.id === "startGame") {
        if (state.editingSetup) {
          ...
          if (LIVE_GAME_ID && LIVE_CAN_EDIT) {
            ...
            if (!ok) {
              await uiAlert(liveSaveStatus || "Live save gagal. Perubahan belum tersimpan ke server.", { title: "Gagal menyimpan", tone: "warn" });
              return;
            }
          } else {
            save();
        ...
        if (entered.length < 2) { await uiAlert("Minimal isi 2 nama player.", { title: "Player belum lengkap" }); return; }
      ...
      if (b.id === "resetAutoBanker") {
        if (manualBankerRequired(state.active)) {
          await uiAlert("Hole Par 3 wajib bandar manual, tidak bisa pakai auto.", { title: "Bandar manual", tone: "warn" });
          return;
        }
