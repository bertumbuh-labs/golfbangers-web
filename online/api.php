...
try {
    ...
    if ($action === 'list') {
        ...
        if (!hash_equals($viewerKey, $key)) {
            respond(['ok' => false, 'error' => 'Viewer password salah.'], 403);
        }
    ...
    if ($action === 'create') {
        ...
        $id = gameId();
        $stmt = $db->prepare('INSERT INTO games (id, title, edit_token_hash, state_json, expires_at) VALUES (?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL 24 HOUR))');
    ...
    if ($action === 'update') {
        $data = body();
        $token = trim($data['token'] ?? '');
        if (!$token || !password_verify($token, $game['edit_token_hash'])) {
